...
router.get('/now', (req, res, next) => {
  ...
  Matkul.getPresentMatkul(
    labdasar,
    { hari: presentDate.getDay() },
    (err, matkul) => {
      if (err) throw err;
      if (matkul.length > 0) {
        let presentMatkul;
        matkul.forEach(element => {
          ...
          let menitPresent =
            presentDate.getHours() * 60 + presentDate.getMinutes();
          if (
            menitMatkul.mulai <= menitPresent &&
            menitMatkul.selesai >= menitPresent
          ) {
